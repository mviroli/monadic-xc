...
import scafi.FreeMonads.Free
import scafi.NValuesAggregateTestUtilities.{*, given}
import scafi.NValueAggregates.{*, given}

class NValueAggregateTest extends org.scalatest.funsuite.AnyFunSuite:

  extension [A](a: A) def nv: NValue[A] = NValue(a, Map.empty)
  def counter(initial: Int): Aggregate[NValue[Int]] = rep(initial)(for i <- _ yield i + 1)

  test("Trees of a constant rep"):
    val ag: Aggregate[NValue[Int]] = rep(5)(identity)
